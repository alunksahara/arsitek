...
export async function PATCH(request: Request, context: Context) {
  ...
  if (!validUuid(id)) return jsonError("ID portfolio tidak valid.", 400);
  ...
  try {
    ...
    if ("sort_order" in body) {
      ...
      patch.sort_order = Math.max(0, Math.trunc(sortOrder));
    }
    ...
    if (error) {
      ...
      if (error.code === "23505") return jsonError("Slug portfolio sudah digunakan.", 409);
