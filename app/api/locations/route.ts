...
async function readJson(request: Request) {
  try {
    ...
    if (!data || typeof data !== "object" || Array.isArray(data)) {
      return { data: null, error: "Body JSON harus berupa object" };
    }
...
export async function PATCH(request: Request) {
  ...
  if (!id) {
    return jsonError("ID lokasi wajib diisi", 400);
  }
  ...
  if (oldError || !oldData) {
    return jsonError("Lokasi tidak ditemukan", 404);
  }
...
async function writeAudit(
  ...
  if (error) {
    console.error("[LOCATION AUDIT]", error);
  }
}
