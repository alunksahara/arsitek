  ...
  return (
    <main className="min-h-screen bg-[#f4f1eb] py-8 md:py-16">
      <div className="container-main">
        <a href="/" className="inline-flex min-h-11 items-center gap-2 text-xs font-semibold uppercase tracking-[0.2em] text-[#343731]">
          ...
          {success ? (
            ...
            <form onSubmit={submitForm} className="mt-10 grid gap-8 md:mt-14">
              <div className="grid gap-8 md:grid-cols-2">
                <label className="grid gap-2 text-sm font-semibold text-[#343731]">Nama lengkap
                  <input name="name" required autoComplete="name" placeholder="Nama Anda" className="min-h-14 border-b border-[#cfc9be] bg-transparent px-0 py-3 outline-none transition-colors placeholder:text-[#9a968d] focus:border-[#24563b]" />
                </label>
                <label className="grid gap-2 text-sm font-semibold text-[#343731]">WhatsApp / Telepon
                  <input name="phone" required autoComplete="tel" inputMode="tel" placeholder="08xxxxxxxxxx" className="min-h-14 border-b border-[#cfc9be] bg-transparent px-0 py-3 outline-none transition-colors placeholder:text-[#9a968d] focus:border-[#24563b]" />
                </label>
              </div>

              <div className="grid gap-8 md:grid-cols-2">
                <label className="grid gap-2 text-sm font-semibold text-[#343731]">Email
                  <input name="email" type="email" autoComplete="email" placeholder="nama@email.com" className="min-h-14 border-b border-[#cfc9be] bg-transparent px-0 py-3 outline-none transition-colors placeholder:text-[#9a968d] focus:border-[#24563b]" />
                </label>
                <label className="grid gap-2 text-sm font-semibold text-[#343731]">Jenis proyek
                  <select name="projectType" required defaultValue="" className="min-h-14 border-b border-[#cfc9be] bg-transparent px-0 py-3 outline-none focus:border-[#24563b]">
                    <option value="" disabled>Pilih jenis proyek</option>
                    <option>Rumah baru</option><option>Renovasi</option><option>Interior</option><option>Komersial</option><option>Lainnya</option>
                  </select>
                </label>
              </div>

              <label className="grid gap-2 text-sm font-semibold text-[#343731]">Perkiraan anggaran
                <select name="budget" defaultValue="" className="min-h-14 border-b border-[#cfc9be] bg-transparent px-0 py-3 outline-none focus:border-[#24563b]">
                  <option value="" disabled>Pilih kisaran anggaran</option>
                  <option>Di bawah Rp 500 juta</option><option>Rp 500 juta – Rp 1 miliar</option><option>Rp 1 – 2 miliar</option><option>Rp 2 – 5 miliar</option><option>Di atas Rp 5 miliar</option>
                </select>
              </label>

              <label className="grid gap-2 text-sm font-semibold text-[#343731]">Ceritakan proyek Anda
                <textarea name="message" rows={6} placeholder="Contoh: luas tanah, lokasi, kebutuhan ruang, gaya yang disukai, target waktu, atau hal lain yang penting bagi Anda." className="resize-y border border-[#cfc9be] bg-white/40 p-4 font-normal outline-none transition-colors placeholder:text-[#9a968d] focus:border-[#24563b]" />
              </label>
