...
export default function LocationManager() {
  ...
  function reset() { setForm(emptyLocation); setMessage(""); }
    ...
    <main className="min-h-screen bg-neutral-50 px-4 py-8 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-7">
          <div className="mb-6 flex items-center justify-between"><h2 className="text-xl font-semibold">{form.id ? "Edit lokasi" : "Tambah lokasi"}</h2>{form.id && <button onClick={reset} className="text-sm text-neutral-500">Batal edit</button>}</div>
        ...
        <section className="mt-8 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm sm:p-7">
          ...
          {loading ? <p className="mt-4 text-sm text-neutral-500">Memuat...</p> : <div className="mt-5 divide-y divide-neutral-200">{items.map((item) => <div key={item.id} className="flex flex-wrap items-center justify-between gap-4 py-4"><div><div className="font-medium">{item.city}</div><div className="mt-1 text-xs text-neutral-500">/{item.slug} · {item.published ? "Published" : "Draft"}</div></div><div className="flex gap-2"><a target="_blank" rel="noreferrer" href={`/${item.slug}`} className="rounded-full border border-neutral-300 px-3 py-2 text-xs">Lihat</a><button onClick={() => edit(item)} className="rounded-full border border-neutral-300 px-3 py-2 text-xs">Edit</button><button onClick={() => remove(item.id)} className="rounded-full border border-red-200 px-3 py-2 text-xs text-red-700">Hapus</button></div></div>)}</div>}
        </section>
      </div>
    </main>
  );
