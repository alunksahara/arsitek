...
export default function ArchitectureEstimator() {
  const [projectType, setProjectType] =
    useState<ProjectType>("Rumah Baru");

  const [designLevel, setDesignLevel] =
    useState<DesignLevel>("Signature");

  const [area, setArea] = useState<number>(
    FALLBACK_SETTINGS.min_area
  );

  const [settings, setSettings] =
    useState<EstimatorSettings>(FALLBACK_SETTINGS);
  ...
  useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        if (!response.ok) {
          throw new Error("Gagal mengambil estimator settings");
        }
        ...
        if (mounted && result?.settings) {
          setSettings({
            essential_rate:
              Number(result.settings.essential_rate),

            signature_rate:
              Number(result.settings.signature_rate),

            premium_rate:
              Number(result.settings.premium_rate),

            rumah_baru_multiplier:
              Number(
                result.settings.rumah_baru_multiplier
              ),

            renovasi_multiplier:
              Number(
                result.settings.renovasi_multiplier
              ),

            villa_multiplier:
              Number(
                result.settings.villa_multiplier
              ),

            commercial_multiplier:
              Number(
                result.settings.commercial_multiplier
              ),

            min_range_multiplier:
              Number(
                result.settings.min_range_multiplier
              ),

            max_range_multiplier:
              Number(
                result.settings.max_range_multiplier
              ),

            min_area:
              Number(result.settings.min_area),
          });
        }
      } catch (error) {
        console.error(
  ...
  useEffect(() => {
    if (
      area < settings.min_area ||
      !Number.isFinite(area)
    ) {
      setArea(settings.min_area);
    }
  ...
  return (
    <section
      id="estimator"
      className="relative overflow-hidden bg-[#f4efe5] py-20 sm:py-24"
    >
            ...
            <div className="mt-7">
              ...
              <div className="mt-3 flex items-center gap-3">
                <input
                  ...
                  className="w-full accent-[#255c45]"
                />
                ...
                <input
                  ...
                  className="w-24 rounded-xl border border-black/10 bg-white px-3 py-2 text-center text-sm font-semibold text-black outline-none focus:border-black/40"
                />
              </div>
